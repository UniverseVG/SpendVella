...
import { GroupBalance, User, UserType } from "@/types";
 ...
 * }
 */
function GroupBalances({ balances } : { balances: GroupBalance['balances'] }) {
  ...
  /* ───── helpers ───────────────────────────────────────────────────────── */
  const me = balances.find((b: UserType) => b.id === currentUser._id);
  if (!me) {
    return (
      <div className="text-center py-4 text-muted-foreground">
  ...
  const owedByMembers = me.owedBy
    .map(({ from, amount }: { from: string; amount: number }) => ({
      ...userMap[from],
      amount,
    }))
    .sort((a: UserType, b: UserType) => (b.amount ?? 0) - (a.amount ?? 0));

  // Whom do I owe?
  const owingToMembers = me.owes
    .map(({ to, amount }: { to: string; amount: number }) => ({
      ...userMap[to],
      amount,
    }))
    .sort((a: UserType, b: UserType) => (b.amount ?? 0) - (a.amount ?? 0));
        ...
        <div className="space-y-4">
          ...
          {owedByMembers.length > 0 && (
            <div>
              ...
              <div className="space-y-3">
                {owedByMembers.map((member: UserType) => (
                  <div
                    key={member.id}
          ...
          {owingToMembers.length > 0 && (
            <div>
              ...
              <div className="space-y-3">
                {owingToMembers.map((member: UserType) => (
                  <div
                    key={member.id}
                    className="flex items-center justify-between"
                  >
