...
function ExpenseList({
  ...
  const getUserDetails = (userId: string, expense?: ExpenseType) => {
  ...
  const canDeleteExpense = (expense: ExpenseType) => {
  ...
  const handleDeleteExpense = async (expense: ExpenseType) => {
    ...
    try {
      await deleteExpense.mutate({ expenseId: expense._id as string });
  ...
  return (
    <div className="flex flex-col gap-4">
      {expenses.map((expense: ExpenseType) => {
        ...
        const category = getCategoryById(expense.category as string);
