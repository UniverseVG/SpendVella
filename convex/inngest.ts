...
// Get users with outstanding debts (1-to-1 only) means groupId === undefined in expenses/settlements
// Result: [{ _id, name, email, debts: [{ userId, name, amount, since }] }]
...
export const getUsersWithExpenses = query({
  handler: async (ctx) => {
    ...
    oneMonthAgo.setMonth(now.getMonth() - 1); // 1 month ago
