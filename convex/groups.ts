...
export const getGroupOrMembers = query({
  ...
  handler: async (
    ...
    } | null; // null if no specific group requested
    groups: {
      id: Id<"groups">;
      name: string;
      description: string;
  ...
  handler: async (ctx, { groupId }) => {
    ...
    const memberDetails = await Promise.all(
      group.members.map(async (m) => {
        const u: Doc<"users"> | null = await ctx.db.get(m.userId);
    ...
    const ledger: Record<Id<"users">, Record<Id<"users">, number>> = {};
    ids.forEach((a) => {
      ledger[a] = {};
      ids.forEach((b) => {
        if (a !== b) ledger[a][b] = 0;
