...
async function createGroups(ctx: any, users: any) {
  const now = Date.now();
...
async function createOneOnOneExpenses(ctx: any, users: any) {
  const now = Date.now();
...
async function createGroupExpenses(ctx: any, users: any, groups: any) {
  const now = Date.now();
  const oneWeekAgo = now - 7 * 24 * 60 * 60 * 1000;
  const twoWeeksAgo = now - 14 * 24 * 60 * 60 * 1000;
...
async function createSettlements(
  ...
  const cabExpense = oneOnOneExpenses.find(
    (expense: any) => expense.description === "Cab ride to airport"
  );

  // Find some group expenses to settle
  const hotelExpense = groupExpenses.find(
    (expense: any) => expense.description === "Hotel reservation"
  );

  const coffeeExpense = groupExpenses.find(
    (expense: any) => expense.description === "Coffee and snacks"
  );
